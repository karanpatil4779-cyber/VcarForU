...
const AgencyLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_URL}/auth?action=login&type=agency`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center py-12 px-4">
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Agency Login</h1>
          <p className="text-gray-500">Sign in to manage your vehicles and rentals</p>
        </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-4">
          New agency? <Link to="/agency-signup" className="text-green-600 font-semibold hover:underline">Register</Link>
        </p>
      </div>
    </div>
  );
